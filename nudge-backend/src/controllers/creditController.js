...
const creditController = {
  async recordSpin(req, res) {
    ...
    try {
      ...
      if (userId) {
        // Check streak for bonus credits
        ...
        let streakBonus = 0;
        if (streak >= 30) streakBonus = 2;
        else if (streak >= 7) streakBonus = 1;
        creditsEarned += streakBonus;

        await pool.query(
          'UPDATE users SET social_credits = social_credits + $1 WHERE id = $2',
          [creditsEarned, userId]
        );

        await pool.query(
          'INSERT INTO spin_history (user_id, credits_earned, is_winning) VALUES ($1, $2, $3)',
          [userId, creditsEarned, isWinning]
        );

        const result = await pool.query(
          'SELECT social_credits FROM users WHERE id = $1',
          [userId]
        );

        const io = req.app.get('io');
        io.emit('creditUpdate', {
          userId,
          newBalance: result.rows[0].social_credits,
          creditsEarned,
        });
        ...
        res.json({
          creditsEarned,
          isWinning,
          message: 'Sign up to save your credits!',
        });
      }
    } catch (error) {
    ...
    try {
      const statsResult = await pool.query(
        `SELECT 
          ...
          COUNT(*) FILTER (WHERE is_winning) as winning_spins
      ...
      res.json({
        ...
        winRate: stats.total_spins > 0
          ? ((parseInt(stats.winning_spins) / parseInt(stats.total_spins)) * 100).toFixed(2)
          : 0,
        streak,
      });
    } catch (error) {
